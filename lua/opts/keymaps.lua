...
local function get_root()
  ...
  local buf_dir = buf_path ~= '' and vim.fn.fnamemodify(buf_path, ':h')
    or vim.uv.cwd()
...
map('n', '<leader>mt', ':JasonHistory<CR>', { desc = 'Jason history' })
map('n', '<F6>', ':Jason<CR>', { desc = 'Jason' })
...
map(
  'n',
  '<leader>tt',
  ':ToggleTerm direction=float dir=%:p:h<CR>i',
  { desc = 'Terminal (float)' }
)
map(
  'n',
  '<leader>th',
  ':ToggleTerm size=12 direction=horizontal dir=%:p:h<CR>i',
  { desc = 'Terminal (h)' }
)
map(
  'n',
  '<leader>tv',
  ':ToggleTerm size=80 direction=vertical dir=%:p:h<CR>i',
  { desc = 'Terminal (v)' }
)
map({ 'n', 't' }, '<C-\\>', function()
