return {
	{
		...
		opts = {
			...
			buttons = {
				{ label = "Source", url = "https://github.com/Jlesster/nvimConf" },
			},
		},
	},
	{
		...
		config = function()
			...
			local function get_project_term()
				...
				if not terminals_by_dir[dir] then
					terminals_by_dir[dir] = Terminal:new({
						hidden = true,
						direction = "horizontal",
						size = 15,
						dir = dir,
	...
	},
	{
		...
		config = function()
			require("marvin").setup({
				terminal = {
					position = "background", -- float | split | vsplit | background
					size = 0.4,
